...
class PreSaleComponentDetail extends Component {
  ...
  renderDataCitizenship = () => {
    const letDataCitizenship = [];
    _.forEach(dataCitizenship, (value, index) => {
      letDataCitizenship.push(<MenuItem value={value.value} key={index} primaryText={value.text} />);
    });

    return letDataCitizenship;
  }
          ...
          <div className="row">
            <div className="col-sm-6">
              <SelectField
                ...
                name="citizenship"
                value={this.state.currentPreSale.citizenship || ""}
                onChange={(e,b,c) => { this.onChangeCurrentPreSale({citizenship: c})}}
                fullWidth
              >
