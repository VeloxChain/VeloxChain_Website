...
export function generateFonts(done) {
  ...
  const fontminVendor = new Fontmin()
    .src(config.fonts)
    .use(Fontmin.otf2ttf({
      clone: true
    }))
    ...
    .dest(`${config.src}/styles/fonts`);

  return fontminVendor.run((err) => {
    if (err) {
      console.log(err);
    }
    fontmin.run((err) => {
...
export function bundleVendorCss(done) {
  const updatedConfig = getConfig();
  concatenateFiles({
    src: updatedConfig.vendorCss,
    dest: `${updatedConfig.dist}/styles`,
